...
when isMainModule:
  ...
  let mappedMemory = mapMemory(device, uniformBufferMemory, 0.VkDeviceSize, uniformSize, 0.VkMemoryMapFlags)
  copyMem(mappedMemory, ubo.addr, uniformSize.int)
  ...
  recordCommandBuffer(
    ...
    groupCountX = ceilDiv(width, workgroupSize.x),
    groupCountY = ceilDiv(height, workgroupSize.y)
  )
